...
The chibiTools.py library is dependent on pySerial  which is not included with the standard install of Python. This library has been tested and created for Python 2.7.
...
#### Initialize Radio
```
chibiTools.initialize(comPort, channel = 11, frameCallback = None, commandCallback = None, comSpeed = 57600)
```
...
***comSpeed*** --- (Optional) Is an integer value to set the serial baud rate between the computer and radio. This should never need to be changed, unless the chibiTools Arduino sketch is modified to have another value.

#### Terminate All
```
chibiTools.terminate()
```
...
```
chibiTools.initOutPcapFile(aFile)
```
...
```
chibiTools.writeFrameToPcap(aFrame)
```
...
```
chibiTools.initInPcapFile(aFile)
```
...
```
chibiTools.getFrameFromPcap(index)
```
...
#### Get Next Channel
```
chibiTools.getNextChannel(currChannel, upDown)
```
...
#### Change Channel
```
chibiTools.changeChannel(aChannel)
```
...
#### RSSI To Percent
```
chibiTools.rssiToPercent(aByte)
```
...
#### Pretty Hex
```
chibiTools.prettyHex(aString, newLine = 16)
```
...
def dataEvent(aFrame):
    ...
    chibiTools.writeFrameToPcap(aFrame)
    print chibiTools.prettyHex(aFrame.frame)
    print "RSSI: ", chibiTools.rssiToPercent(aFrame.rssi), “%”

if __name__ == '__main__':
    curChnl = 11
    chibiTools.initOutPcapFile("C:\\temp\\test.pcap")
    chibiTools.initialize("COM9", curChnl, dataEvent)
    for i in range(16):
        print "Changing Channel to ", curChnl
        time.sleep(15)
        curChnl = chibiTools.getNextChannel(curChnl, "+")
        chibiTools.changeChannel(curChnl)
